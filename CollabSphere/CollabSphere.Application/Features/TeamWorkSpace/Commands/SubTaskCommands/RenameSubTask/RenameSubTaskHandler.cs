...
{
    public class RenameSubTaskHandler : CommandHandler<RenameSubTaskCommand>
    {
        private readonly IUnitOfWork _unitOfWork;
        public RenameSubTaskHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }

        protected override async Task<CommandResult> HandleCommand(RenameSubTaskCommand request, CancellationToken cancellationToken)
        {
                            ...
                            if (task.TaskId == request.TaskId)
                            {
                                ...
                                if (subTasksOfCard.Count > 0)
                                {
                                    foreach (var sub in subTasksOfCard)
                                    {
                                        ...
                                        {
                                            sub.SubTaskTitle = request.SubTaskNewTitle;
        ...
        protected override async Task ValidateRequest(List<OperationError> errors, RenameSubTaskCommand request)
        {
            //Find team workspace
