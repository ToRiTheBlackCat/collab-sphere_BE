...
{
    public class MarkUnMarkdoneSubTaskHandler : CommandHandler<MarkUnMarkdoneSubTaskCommand>
    {
        private readonly IUnitOfWork _unitOfWork;
        public MarkUnMarkdoneSubTaskHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }

        protected override async Task<CommandResult> HandleCommand(MarkUnMarkdoneSubTaskCommand request, CancellationToken cancellationToken)
        {
                            ...
                            if (task.TaskId == request.TaskId)
                            {
                                ...
                                if (subTasksOfCard.Count > 0)
                                {
                                    foreach (var sub in subTasksOfCard)
                                    {
                                        ...
                                        {
                                            sub.IsDone = request.IsDone;

                                            _unitOfWork.SubTaskRepo.Update(sub);
        ...
        protected override async Task ValidateRequest(List<OperationError> errors, MarkUnMarkdoneSubTaskCommand request)
        {
            //Find team workspace
