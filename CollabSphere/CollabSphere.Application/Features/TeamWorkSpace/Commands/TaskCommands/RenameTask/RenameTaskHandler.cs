...
{
    public class RenameTaskHandler : CommandHandler<RenameTaskCommand>
    {
        private readonly IUnitOfWork _unitOfWork;
        public RenameTaskHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }

        protected override async Task<CommandResult> HandleCommand(RenameTaskCommand request, CancellationToken cancellationToken)
        {
                ...
                if (foundCard != null)
                {
                    ...
                    if (tasksOfCard.Count > 0)
                    {
                        foreach (var task in tasksOfCard)
                        {
                            ...
                            {
                                task.TaskTitle = request.NewTitle;
        ...
        protected override async Task ValidateRequest(List<OperationError> errors, RenameTaskCommand request)
        {
            //Find team workspace
